...
@section('content')
    ...
    <nav class="mb-4" aria-label="breadcrumb">
        <ol class="breadcrumb flex-lg-nowrap">
            ...
            <li class="breadcrumb-item text-nowrap" aria-current="page">
                <a class="text-nowrap" href="{{ url('dashboard/records') }}">
                    记录列表
                </a>
            </li>
        </ol>
    ...
    <div class="card mb-5">
        <div class="card-body">
            <form action="{{url('/dashboard/records/'.$data->id)}}" method="post" autocomplete="off">
                @csrf
                ...
                <div>
                    ...
                    <select class="form-select" id="platform_id" name="platform_id">
                        @foreach($platforms as $item)
                            <option @if($item->id === $data->platform_id) selected
                ...
                <div class="mt-3">
                    ...
                    <input class="form-control" type="number" id="uid" name="uid" value="{{$data->uid}}">
                </div>
                <div class="mt-3">
                            ...
                            <optgroup label="{{$parent->title}}">
                                @if($parent->children)
                                    @foreach($parent->children as $children)
                                        <option @if($children->id === $data->category_id) selected
                ...
                <div class="mt-3">
                    ...
                    <input class="form-range" type="range" id="rate" name="rate" min="1" max="5"
                           value="{{$data->rate}}">
                </div>
                <div class="mt-3">
